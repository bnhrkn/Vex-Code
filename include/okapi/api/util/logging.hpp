...
class Logger {
 public:
  ...
  /**
   * A logger that opens the input file by name. If the file contains `/ser/`,
   * the file will be opened in write mode. Otherwise, the file will be opened
   * in append mode. The file will be closed when the logger is destructed.
   *
   * @param itimer A timer used to get the current time for log statements.
   * @param ifileName The name of the log file to open.
   * @param ilevel The log level. Log statements more verbose than this level
   * will be disabled.
   */
  ...
  /**
   * A logger that uses an existing file handle. The file will be closed when
   * the logger is destructed.
   *
   * @param itimer A timer used to get the current time for log statements.
  ...
  void debug(T ilazyMessage) noexcept {
    if (isDebugLevelEnabled() && logfile && timer) {
      ...
      fprintf(logfile, "%ld (%s) DEBUG: %s\n",
              static_cast<long>(timer->millis().convert(millisecond)),
              CrossplatformThread::getName().c_str(), ilazyMessage().c_str());
    }
  }
  ...
  /**
   * Sets a new default logger. OkapiLib classes use the default logger unless
   * given another logger in their constructor.
   *
...
#else
    defaultLogger = std::make_shared<Logger>(
        std::make_unique<Timer>(), "/ser/sout", Logger::LogLevel::warn);
#endif
  }
...
static DefaultLoggerInitializer
